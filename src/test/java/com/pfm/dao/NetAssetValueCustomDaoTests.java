...
public class NetAssetValueCustomDaoTests {
	...
	public void givenNAVInstance_whenFindByDateInvoked_thenReturnListOfNAVInstance() {
		...
		cal.set(2023, 10, 10);
		jdbcTemplate.update("insert into nav (scheme_id,nav_date,nav) values(?,?,?)", "SM001", new Date(), BigDecimal.ONE);

		// when ( action / behavior tested ) 
		List<NetAssetValueDTO> list = jdbcTemplate.query("select * from nav a where a.nav_date>?", rowMapper,cal.getTime());
